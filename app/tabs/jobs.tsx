...
function JobCard({ job }: { job: any }) {
  ...
  return (
    <View className="w-full max-w-md bg-white rounded-2xl shadow-lg p-6 mb-8 border border-gray-100">
      {/* Top Icon */}
      <View className="items-center -mt-12 mb-2">
        <View
          className={`${bg} rounded-full p-4 shadow-md border border-white`}
        >
          {icon}
        </View>
      </View>
      ...
      <Text className="text-xl font-bold text-gray-900 text-center mb-1">
        <Text className="font-extrabold">{job.title}</Text>
      </Text>
      {/* Star rating */}
      ...
      >
        ...
        <Text className="text-white text-center text-xs mt-1">per day</Text>
      </LinearGradient>
      {/* Info blocks */}
...
export default function Jobs() {
  return (
    <ScrollView
      ...
      contentContainerStyle={{ alignItems: "flex-start", paddingBottom: 32 }}
    >
      {mockJobs.map((job) => (
